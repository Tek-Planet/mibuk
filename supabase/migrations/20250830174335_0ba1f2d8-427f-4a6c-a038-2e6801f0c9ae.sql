...
-- RLS policies
create policy if not exists "Users can view their own supplier payments"
  on public.supplier_payments for select
  using (auth.uid() = user_id);

create policy if not exists "Users can create their own supplier payments"
  on public.supplier_payments for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own supplier payments"
  on public.supplier_payments for update
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own supplier payments"
...
-- Note: Supabase's default publication is supabase_realtime
-- Safe to attempt add
begin;
  alter publication supabase_realtime add table public.supplier_payments;
exception when others then
  -- ignore if it already exists
  raise notice 'supplier_payments already in publication or publication missing';
end;